...
public class ComplementaryPairs {
    ...
    /**
     * Driver program for testing the ComplementaryPairs. Reads libs as input from a test file
     * ".\libs\ComplementaryPairs.txt" Writes output at the end of the same file
     *
     * @param args
     */
        ...
        try (
            ...
            while (inputScanner.hasNext()) {
                ...
                if (nxtLiteral != null && !nxtLiteral.isEmpty()) {
                    ...
                    Arrays.stream(inputSplit[0].split(","))
                            .forEach(x -> inputArr.add(Integer.valueOf(x.trim())));
                    int[] testArr = new int[inputArr.size()];
                    for (int i = 0; i < inputArr.size(); i++) {
                        testArr[i] = inputArr.get(i);
                    }
                    outputWriter.write(
                            complementaryPairsFinder.findKComplementaryPairs(testArr, K) + "\n");
                }
            }
    ...
    private static class IndexPair {
        ...
        @Override
        /**
         * a pair is equal if it has same left and right OR it is a duplicate these 2 pairs are
         * duplicate: (2,1) and (1,2)
         */
