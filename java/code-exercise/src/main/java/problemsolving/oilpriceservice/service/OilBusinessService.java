package problemsolving.oilpriceservice.service;

import problemsolving.oilpriceservice.calc.*;
import problemsolving.oilpriceservice.jpa.repository.JPARepository;
import problemsolving.oilpriceservice.model.OilType;
import problemsolving.oilpriceservice.model.OilTypeRecord;
import problemsolving.oilpriceservice.model.OilTypeTransactionRecord;
import problemsolving.oilpriceservice.model.Side;
...
public class OilBusinessService {
    ...
    private final JPARepository<OilTypeRecord> oilTypeRepository;
    private final JPARepository<OilTypeTransactionRecord> oilTransactionRepository;
     ...
     * @return
     */
    public boolean recordTransaction(OilTypeRecord oilData, Timestamp createsTimestamp,
                                     double quantity, Side side, double price, boolean isActiveRecord) {

        OilTypeTransactionRecord oilTypeTransactionRecord = new OilTypeTransactionRecord(oilData,
                createsTimestamp, quantity, side, price, isActiveRecord);
    ...
    private double getRevenueYield(double price, OilTypeRecord oilTypeRecord) {
        revenueYieldFunction =
                (RevenueYieldFunction) (oilTypeRecord.getOilType().equals(OilType.PREMIUM.getType())
                        ? CalcFunctionFactory.getPremiumRevenueYieldFunction(price,
                        oilTypeRecord.getVariableRevenue(),
                        oilTypeRecord.getOilBarrelValue())
                        : CalcFunctionFactory.getStandardRevenueYieldFunction(price,
    ...
    private double getPriceEarningsRatio(double price, OilTypeRecord oilTypeRecord) {
        priceEarningsRatioFunction = (RatioFunction) CalcFunctionFactory
                .getPriceEarningsRatioFunction(price, oilTypeRecord.getFixedRevenue());
        return priceEarningsRatioFunction.getRatio();
    }

    private double getVolumeWeightedOilPrice(int lastNMinutes,
                                             OilTypeTransactionRecord oilTypeRecord) {
        ...
        result.entrySet().stream().forEach(x -> {
            if (oilTypeRecord.getOilTypeRecord().getOilId()
                    .equals(x.getValue().getOilTypeRecord().getOilId())
                    && (Calendar.getInstance().toInstant().getNano() - x.getValue()
                    .getCreatedTimestamp().toInstant().getNano()) <= lastNMinutes * 60 * 60
                    * 60) {
        ...
        volumeWeightedPriceFunction = (WeightedPriceFunction) CalcFunctionFactory
                .getVolumeWeightedPriceFunction(prices, quantities);

        return priceEarningsRatioFunction.getRatio();
    }

    private double getGeometricMean(OilTypeTransactionRecord oilTypeTxnRecord) {
        Set<Map.Entry<Object, OilTypeTransactionRecord>> entries =
                oilTransactionRepository.fetchAll().entrySet();
        double[] prices = new double[entries.size()];
        int c = 0;

        for (Map.Entry<Object, OilTypeTransactionRecord> e : entries) {
            if (e.getValue().getOilTypeRecord().getOilId()
                    .equals(oilTypeTxnRecord.getOilTypeRecord().getOilId())) {
                prices[c] = e.getValue().getPrice();
                c += 1;
            }
        }
        ...
        geometricMeanFunction =
                (MeanFunction) CalcFunctionFactory.getGeometricMeanFunction(pricesNonZero);
