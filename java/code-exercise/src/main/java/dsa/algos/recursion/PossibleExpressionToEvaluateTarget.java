...
/**
 * Print all possible expressions that evaluate to a target Given a string that
 * contains only digits from 0 to 9, and an integer value, target. Find out how
 * many expressions are possible which evaluate to target using binary operator
...
public class PossibleExpressionToEvaluateTarget {
	...
	private static String evalNow(List<String> pem) {
		...
		/*
		 * return evalNow(pem, i + 1, tgt + Integer.valueOf(pem.get(i)), str + "+" +
		 * pem.get(i));
		 */
