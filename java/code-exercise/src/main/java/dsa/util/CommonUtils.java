...
public class CommonUtils {
	...
	public static String[] addStringToArray(final String[] array, final String str) {
		 ...
		 * if(s.size()==1){ v[i] = x[i]*x[i]; } else { v[i]=s.first() * s.last(); } }
		 * else if(operations[i]=="pop"){ s.remove(x[i]); if(s.size()==1){ v[i] =
		 * x[i]*x[i]; } else { v[i]=s.first() * s.last(); } } } return v;
		 */
	}
	...
	/**
	 * convert the array into string separated by default delimiter which is comma
	 * separator
	 * 
	 * @param arr
	 * @return
	...
	public static boolean checkRotation(final String str1, final String str2) {
		...
		if ((str1.length() == str2.length())
				&& (isSubstring(str1, (str2.concat(str2))) || isSubstring(str2, (str1.concat(str1)))))
			return true;
