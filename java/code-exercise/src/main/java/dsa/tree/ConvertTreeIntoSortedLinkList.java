...
public class ConvertTreeIntoSortedLinkList {
	...
	private static void treeToList(dsa.tree.Node<Integer> node, LinkedList<Integer> list)
			throws Exception {
		dsa.tree.Node<Integer> root = node;
		...
		Stack<dsa.tree.Node<Integer>> s = new Stack();
		Queue<Integer> resultQ = new Queue<>();

		dsa.tree.Node leftNode = null;
		...
		while (root != null || !s.isEmpty()) {
			...
			if (!s.isEmpty()) {
				dsa.tree.Node<Integer> tmpNd = s.pop();
				if (null != tmpNd) {
