...
public class TestBinarySearchTree {
	...
	public void testBasicOperations() {
		...
		try {
			...
			System.out.format("[Given node = %d][Left child = %d][Right child = %d]\n", tree.getRoot().getValue(),
					tree.getRoot().getLeftNode().getValue(), tree.getRoot().getRightNode().getValue());

			// search test
			final Node<Integer> node = tree.searchNode(20);
			System.out.format("[Given node = %d][Left child = %d][Right child = %d]\n", node.getValue(),
					node.getLeftNode().getValue(), node.getRightNode().getValue());
			...
			System.out.format("[Given node = %d][Left child = %d][Right child = %d]\n", nodeJustDeleted.getValue(),
					nodeJustDeleted.getLeftNode().getValue(), nodeJustDeleted.getRightNode().getValue());
			...
			System.out.format("[Given node = %d][Left child = %d][Right child = %d]\n", tree.searchNode(30).getValue(),
					(tree.searchNode(30).getLeftNode() != null ? tree.searchNode(30).getLeftNode().getValue() : 0),
					(tree.searchNode(30).getRightNode() != null ? tree.searchNode(30).getRightNode().getValue() : 0));
			...
			System.out.format("[Given node = %c][Left child = %c][Right child = %c]\n",
					treeOfCharacter.getRoot().getValue(), treeOfCharacter.getRoot().getLeftNode().getValue(),
					treeOfCharacter.getRoot().getRightNode().getValue());

			// search test
			final Node<Character> nodeOfChar = treeOfCharacter.searchNode('R');
			System.out.format("[Given node = %c][Left child = %c][Right child = %c]\n", nodeOfChar.getValue(),
					nodeOfChar.getLeftNode().getValue(), nodeOfChar.getRightNode().getValue());
	...
	public void testCheckIfTwoNodesAreCousin() {
		...
		System.out.format("Are %d and %d cousins: ", tree.getRoot().getLeftNode().getLeftNode().getValue(),
				tree.getRoot().getRightNode().getLeftNode().getValue());
		System.out.println(TreeUtils.checkIfTwoNodesAreCousins(tree, tree.getRoot().getLeftNode().getLeftNode(),
				tree.getRoot().getRightNode().getLeftNode()));

		System.out.format("Are %d and %d cousins: ", tree.getRoot().getLeftNode().getRightNode().getValue(),
				tree.getRoot().getRightNode().getLeftNode().getValue());
		System.out.println(TreeUtils.checkIfTwoNodesAreCousins(tree, tree.getRoot().getLeftNode().getRightNode(),
				tree.getRoot().getRightNode().getLeftNode()));
		...
		System.out.println(
				TreeUtils.checkIfTwoNodesAreCousins(tree, tree.getRoot().getLeftNode(), tree.getRoot().getRightNode()));
