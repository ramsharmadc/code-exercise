...
public class TreeUtils {
	...
	public static void convertTreeToList() {
		...
		ListADT<Integer> listFromTree = (ListADT<Integer>) TreeUtils.convertToList(tree);
	...
	/**
	 * to print in reverse order from bottom i.e. for the given tree. if In Order is
	 * : 10 20 30 50 70 80 90 Pre Order is : 50 20 10 30 80 70 90 Post Order is : 10
	 * 30 20 70 90 80 50 then Reverse Order would be : 10 30 70 90 20 80 50
	 * 
	 * @param tree
	 * @author ram sharma
	 * @since 02/August/2014
	 */
	public static void printInReverseOrderStartingFromBottom(final BinaryTree<? extends Object> tree) {
	...
	public static boolean checkIfIdenticalTrees(final BinaryTree<Integer> treeOne, final BinaryTree<Integer> treeTwo) {
	...
	public static boolean checkIfTwoNodesAreCousins(final BinaryTree<Integer> tree, final Node<Integer> a,
			final Node<Integer> b) {
		return (levelOf(tree.getRoot(), a) == levelOf(tree.getRoot(), b)) && !(checkIfTwoNodesAreSiblings(tree, a, b))
				? true
				: false;
	}

	public static boolean checkIfTwoNodesAreSiblings(final BinaryTree<Integer> tree, final Node<Integer> a,
			final Node<Integer> b) {
	...
	private static boolean checkIfTwoNodesAreSiblings(final Node<Integer> root, final Node<Integer> a,
			final Node<Integer> b) {
	...
	private static boolean checkIfBinarySearchTree(final Node<Integer> root) {
		...
		if (root.getLeftNode() == null && root.getRightNode() == null) {
			...
			return (root.getValue() > root.getLeftNode().getValue())
					&& (root.getValue() < root.getRightNode().getValue()) && checkIfBinarySearchTree(root.getLeftNode())
	...
	/**
	 * This will create a circular link list which will contain the nodes of the
	 * tree in increasing order(In-Order traversal of tree)
	 * 
	 * @return
	 */
	...
	/**
	 * We can solve this problem using BST properties. We can recursively traverse
	 * the BST from root. The main idea of the solution is, while traversing from
	 ...
	 * (assuming that n1 < n2). So just recursively traverse the BST in, if node's
	 * value is greater than both n1 and n2 then our LCA lies in left side of the
	 * node, if it's is smaller than both n1 and n2, then LCA lies on right side.
	 * Otherwise root is LCA (assuming that both n1 and n2 are present in BST)
	 */
	public static <T> T lowestCommonAncestor(final BinarySearchTree<T> tree, final T node1, final T node2) {
	...
	public static <T> BinaryTree<T> clone(Tree<T> tree) throws InstantiationException, IllegalAccessException {
