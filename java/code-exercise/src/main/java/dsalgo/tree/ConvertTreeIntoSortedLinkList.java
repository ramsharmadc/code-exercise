...
public class ConvertTreeIntoSortedLinkList {
    ...
    private static void treeToList(dsalgo.tree.Node<Integer> node, LinkedList<Integer> list)
            throws Exception {
        dsalgo.tree.Node<Integer> root = node;
        ...
        Stack<dsalgo.tree.Node<Integer>> s = new Stack();
        Queue<Integer> resultQ = new Queue<>();

        dsalgo.tree.Node leftNode = null;
        ...
        while (root != null || !s.isEmpty()) {
            ...
            if (!s.isEmpty()) {
                dsalgo.tree.Node<Integer> tmpNd = s.pop();
                if (null != tmpNd) {
